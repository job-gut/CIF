...
export function alertAboutUpdates(): void {
	if (thisACisLastestVersion() === false) {
		...
		for (const pl of ops) {
			...
			CIF.announce(`§c[§fCIF§c] §7현재 버전: ${CIFVersion} §l§f-> §r§b최신 버전: ${getNewVersion()}`);
			CIF.announce("§c[§fCIF§c] §6변경 사항: §a" + getWhatsNew());
		};
		...
		CIF.log("변경 사항: ".yellow + getWhatsNew().green);
